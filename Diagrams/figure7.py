...
def main_figure7(evaluation_results, admission_chance_results, insurance_cost_results, used_car_prices_results, metric,
    ...
    Y_SIZE = {
        ...
        "MSE": {"Admission_Chance": 0.007, "Insurance_Cost": 10 ** 8 * 1.2, "Used_Car_Prices": 10 ** 9 * 1.6}.get,
        ...
        for j, model in enumerate(models + ["Ridge", "RandomForest"]):
            ...
            if model in ["Ridge", "RandomForest"]:
                ...
                for config in ['Anonymized_Features', 'Named_Features']:
                    ...
                    if metric == "r2":
                        values = [1 - v for v in values]
                    axes[i].plot(x + width, values, linestyle='-' if config == "Named_Features" else '-.',
                                 label=model.replace("_", " "), color=palette[j])
    ...
    fig.legend(handles, labels, loc='center', bbox_to_anchor=(0.5, -0.06), ncol=4, fontsize=11)
